...
const AddPermition = ({
  ...
  const formItems: FormItem[] = [
    {
      ...
      size: { md: 12 },
      rules: {
        required: "نام مجوز الزامی است",
        minLength: {
          value: 2,
          message: "نام مجوز باید حداقل ۲ کاراکتر باشد",
        },
      },
    },
  ];
  ...
  const onSubmit = (data: FormData) => {
    mutate(
      {
        entity: `permission/${!!editeData ? "update" : "save"}`,
      ...
      },
      {
        onSuccess: (res: any) => {
          if (!!editeData)
            snackbar(
              `به روز رسانی مجوز انتخاب شده با موفقیت انجام شد`,
              "success"
            );
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Box display={"flex"} textAlign={"center"} alignItems={"center"}>
            ...
            <Typography variant="h6">
              {editeData ? `ویرایش مجوز انتخاب شده` : `ایجاد مجوز جدید`}
            </Typography>
          </Box>
