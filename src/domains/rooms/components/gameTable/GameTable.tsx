...
// این زاویه (درجه) پایین میز رو برای خودمون خالی نگه می‌داره تا حریف‌ها روش نیفتن
const BOTTOM_GAP_DEGREES = 70;
// شعاع چیدن بازیکن‌ها نسبت به خود دایره‌ی میز (نه کل استیج)
const SEAT_RADIUS_PERCENT = 42;
...
export default function GameTable({ players, myUserId, currentTurnPlayerId }: GameTableProps) {
  ...
  const getSeatPosition = (index: number) => {
    ...
    if (otherPlayers <= 1) {
      // فقط یک حریف داریم؛ درست بالای میز بذارش
      return {
        top: 50 - SEAT_RADIUS_PERCENT * Math.sin((90 * Math.PI) / 180),
        left: 50,
      };
    }

    // زاویه‌ها رو طوری پخش می‌کنیم که یه بازه‌ی خالی (BOTTOM_GAP_DEGREES) پایین میز بمونه
    const usableArc = 360 - BOTTOM_GAP_DEGREES;
    const startAngle = -90 - usableArc / 2; // از قبل از بالا شروع می‌کنیم، متقارن حول بالای میز
    const angleStep = usableArc / (otherPlayers - 1);
    const angle = startAngle + (index - 1) * angleStep;
    const radians = (angle * Math.PI) / 180;

    return {
      top: 50 + SEAT_RADIUS_PERCENT * Math.sin(radians),
      left: 50 + SEAT_RADIUS_PERCENT * Math.cos(radians),
    };
  };

  return (
    // این لایه‌ی بیرونی جدیده: یه پدینگ دور کل میز می‌ذاره که سیت‌ها همیشه توی همین محدوده بمونن
    <Box
      sx={{
        width: "100%",
        maxWidth: 760,
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <Typography sx={{ fontSize: { xs: 22, sm: 30 }, color: "#d5a94c", textShadow: "0 2px 4px rgba(0,0,0,.5)" }}>
              ♛
            </Typography>
          </Box>
        </Box>
